...
/**
 * 绑定设备管理 - 工地设备绑定 tab 内容
 * 标题（工地名+路径）+ 工具栏（tab/搜索/添加/批量解绑）+ 设备表格
 */
  ...
  <div class="asset-binding">
    ...
    >
      <p class="unbind-tip">
        ...
        <span v-if="unbindBatchMode">确认解绑选中的 {{ selectedCount }} 个设备？解绑后设备将不再属于当前工地。</span>
        <span v-else>确认解绑「{{ unbindTarget?.name }}」？解绑后该设备将不再属于当前工地。</span>
      </p>
    </a-modal>
    ...
    >
      ...
      <p class="add-modal-subtitle">
        {{ activeCategory === 'video' ? '请选择要绑定到该工地的摄像头' : '请选择要绑定到该工地的物联设备' }}
      </p>
